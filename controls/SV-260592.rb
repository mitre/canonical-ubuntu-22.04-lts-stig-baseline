...
Restart the "auditd.service" for the changes to take effect:
  ...
  audit_command = '/etc/audit/plugins.d/au-remote.conf'
  remote_server_command = '/etc/audit/audisp-remote.conf'

  describe package('audispd-plugins') do
    it { should be_installed }
  end
  describe auditd_conf(audit_command) do
    its('active') { should cmp 'yes' }
  end
  describe auditd_conf(remote_server_command) do
    its('remote_server') { should_not be '' }
  end
end
