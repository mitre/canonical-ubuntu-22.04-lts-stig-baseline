...
ca_cert = /etc/ssl/certs/ca-certificates.crt'
  ...
  if input('smart_card_enabled') == false
    impact 0.0
    describe 'Smart card authentication not enabled' do
      skip 'Smart card authentication is not enabled; this control is Not Applicable.'
    end
  else
    describe 'SSSD [sssd] services include pam' do
      subject { (sssd_config['sssd.services'] || '').split(',').map { |s| s.strip.downcase } }
      it { should include('pam') }
    end

    describe 'SSSD [pam] enables certificate authentication' do
      subject { sssd_config['pam.pam_cert_auth'] }
      it { should_not be_nil }
      it { should match(/^(?i:true)$/) }
    end

    describe 'SSSD certificate_verification includes CA checking' do
      subject { sssd_config.content }
      it { should match(/(?mi)^\s*certificate_verification\s*=\s*[^#\n]*\b(?:ca|ca_cert)\b/) }
    end
  end
end
